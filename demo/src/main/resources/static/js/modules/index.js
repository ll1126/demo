layui.config({
        ...
        success: function (data) {
            ...
            if (data.content != null) {
                $.each(data.content, function (i, d) {
                    ...
                    html += "<a href='javascript:;' data-url='" + d.menuUrl + "'>" + d.menuName + "</a>";
                    if (d.children != null && d.children.length > 0) {
                        html += "<dl class='layui-nav-child'>";
                        $.each(d.children, function (j, item) {
                            html += "<dd><a href='javascript:;' data-url='" + item.menuUrl + "'>" + item.menuName + "</a></dd>";
                        })
                        html += "</dl>";
                    }
                    html += "</li>";
    ...
    function initHome() {
        ...
        var url = $("#url").val();
        if(url != null && url != ''){
            one_url = 'demo/recorddetails.html?type=record&id='+url;
        }else {
            // 获取点击菜单上的链接
            var $layui_nav_one = $('.layui-nav-tree').children()[0],
                one_url = $($layui_nav_one).children().data().menuUrl;
        }
